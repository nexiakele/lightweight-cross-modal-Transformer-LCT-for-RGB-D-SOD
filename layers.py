...
def conv_dep_wise(in_c, out_c, d=1, stride=1):
    return nn.Sequential(nn.Conv2d(in_c, in_c, 3, stride=1, padding=d, dilation=d, groups=in_c),
                         nn.BatchNorm2d(in_c),
                         nn.ReLU(),
...
class BasicBlock_dep(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1,group_number=1):
        super(BasicBlock_dep, self).__init__()
        self.conv1 = conv_dep_wise(inplanes, planes, 1, stride)

        self.conv2 = nn.Conv2d(planes, planes, 3, stride=1, padding=1, groups=planes)
