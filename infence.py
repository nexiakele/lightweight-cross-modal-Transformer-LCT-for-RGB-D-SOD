...
#############################################
import tools.Tool1 as tool
import tools.SOD_loader1 as data_loader
...
def val_iter(net, val_data_loader, device, dir_path, eval_tool):
    ...
    #######验证过程
        for i_batch, sample_batched in enumerate(val_data_loader):
            ...
            gt4 = sample_batched['label4'].cuda()
            gt2 = sample_batched['label2'].cuda()
            name = sample_batched['name']
#            h  = sample_batched['height']
#            w  = sample_batched['width']

            out0  = net(rgb, thermal)
            _,_, h, w = gt.shape
...
#####################读取数据###################################################
    ...
    #####################创建目录##########################################################
        time_dir = tool.make_infence_dir(args, subset=dataset)
        eval_tool = Eval_tool()
        eval_tool.reset()
        ...
        print('##########################################')
    print('##########################',epoch,' end ################################')
def get_infence(args, dataset = [1,],model_tpye = 1):      
  if model_tpye == 1:
